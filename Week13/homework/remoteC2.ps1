...
.\Week13\ | export-csv -Path files.csv
...
# Move the files to the remote server
  ...
  Set-SCPItem -ComputerName '192.168.6.71' -Credential (Get-Credential 'john.tiseo@cyber.local') -Path 'C:\Windows\Temp\Files.zip' -Destination '/home/john.tiseo/' -Port '2222'
